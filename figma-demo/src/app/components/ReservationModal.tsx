...
export default function ReservationModal({ onClose, skipLogin = false }: ReservationModalProps) {
  const { t } = useTranslation();
  const [step, setStep] = useState<Step>(skipLogin ? "success" : "login");
        ...
        >
          ...
          {step === "login" && (
            ...
            >
              <div className="text-center mb-8">
                <h2 className="font-display text-2xl mb-2">{t("reservation.title")}</h2>
                <p className="text-sm text-muted-foreground">{t("reservation.loginPrompt")}</p>
              </div>
          ...
          {step === "success" && (
            ...
            >
              ...
              <h2 className="font-display text-2xl mb-2">{t("reservation.successTitle")}</h2>
              <p className="text-muted-foreground mb-4">
                {t("reservation.successMessage")}
              </p>
              <div className="p-4 rounded-xl bg-gold/10 border border-gold/30 mb-8">
                <p className="text-sm text-muted-foreground mb-2">
                  {t("reservation.successNote1")}
                </p>
                <p className="text-sm text-muted-foreground">
                  {t("reservation.successNote2")}
                </p>
              </div>
              ...
              >
                <span className="text-primary-foreground font-medium">{t("reservation.viewAccountButton")}</span>
              </button>
            </motion.div>
          )}
        </motion.div>
