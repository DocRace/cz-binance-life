...
- **书城发码 `/send-login-code` 等联名路由走 `verificationAppAuth`**：Client Service 必须通过 **`VERIFICATION_APP_KEYS`**（JSON 映射）或 **`VERIFICATION_APP_KEY` + `VERIFICATION_APP_SECRET`** 提供密钥，并与书城 BFF 的 **`IPDEX_PARTNER_APP_KEY` / `IPDEX_PARTNER_APP_SECRET` 完全一致**（通常写在 Client 进程的 `.env.market.server`）。若缺省或未对齐，会持续返回 **`code: -10011` Access denied**。
